...
export default function ThreatBriefing({ groups, isNarrow = false }) {
  ...
  const showHistoryPanel = !isNarrow || historyOpen;
  ...
  const inputStyle = {
    ...
    padding: "7px 10px", color: "#c9d1d9", fontSize: 11, fontFamily: "monospace",
  ...
  return (
    <div style={{ flex: 1, display: "flex", overflow: "hidden", background: "#070c12" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ padding: isNarrow ? "10px 14px" : "14px 20px", borderBottom: "1px solid #1e2d3d", background: "#0d1117", flexShrink: 0 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            <div style={{ color: "#3d5168", fontSize: 9, letterSpacing: 2 }}>
              レポート生成フォーム — Claude API
            </div>
            {/* History toggle — narrow only */}
            {isNarrow && (
              <button onClick={() => setHistoryOpen(o => !o)}
                style={{ background: historyOpen ? "#00d4ff22" : "transparent", border: `1px solid ${historyOpen ? "#00d4ff" : "#1e2d3d"}`, borderRadius: 4, padding: "5px 10px", color: historyOpen ? "#00d4ff" : "#4a6378", cursor: "pointer", fontSize: 10, fontFamily: "monospace", minHeight: 34 }}>
                📋 履歴 ({history.length})
              </button>
            )}
          </div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "flex-end" }}>
            <div style={{ flex: isNarrow ? "1 1 100%" : "0 0 auto" }}>
            ...
            <div style={{ flex: isNarrow ? "1 1 calc(50% - 4px)" : "0 0 auto" }}>
              ...
              <select value={period} onChange={e => setPeriod(e.target.value)}
                style={{ ...inputStyle, width: isNarrow ? "100%" : 140 }}>
                {PERIODS.map(p => <option key={p} value={p}>{p}</option>)}
              </select>
            </div>
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap", flex: isNarrow ? "1 1 100%" : "0 0 auto" }}>
              <button onClick={() => generate("executive")} disabled={loading || !hasApiKey}
                style={{ flex: isNarrow ? 1 : "0 0 auto", background: loading ? "#012a15" : "#001a0d", border: `1px solid ${hasApiKey ? "#00ff88" : "#1e2d3d"}`, borderRadius: 4, padding: "8px 12px", color: hasApiKey ? "#00ff88" : "#4a6378", cursor: (loading || !hasApiKey) ? "default" : "pointer", fontSize: 11, fontFamily: "monospace", minHeight: 40, whiteSpace: "nowrap" }}>
                {loading ? "生成中..." : "📋 経営層向け"}
              </button>
              <button onClick={() => generate("soc")} disabled={loading || !hasApiKey}
                style={{ flex: isNarrow ? 1 : "0 0 auto", background: "#0a0d1a", border: `1px solid ${hasApiKey ? "#3b82f6" : "#1e2d3d"}`, borderRadius: 4, padding: "8px 12px", color: hasApiKey ? "#3b82f6" : "#4a6378", cursor: (loading || !hasApiKey) ? "default" : "pointer", fontSize: 11, fontFamily: "monospace", minHeight: 40, whiteSpace: "nowrap" }}>
                {loading ? "生成中..." : "🔍 SOCアナリスト向け"}
              </button>
            </div>
          </div>
          {group && !isNarrow && (
            <div style={{ marginTop: 8, color: "#4a6378", fontSize: 10 }}>
        ...
        {/* Report content */}
        <div className="scroll-touch" style={{ flex: 1, overflowY: "auto", padding: isNarrow ? "12px 14px" : "16px 20px" }}>
          {loading && (
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14, flexWrap: "wrap", gap: 8 }}>
                <div>
                  ...
                  <span style={{ color: "#3d5168", fontSize: 11, marginLeft: 10 }}>
                    {displayReport.groupName} · {displayReport.industry}
                  </span>
                </div>
                <button onClick={exportMd}
                  style={{ background: "transparent", border: "1px solid #1e2d3d", borderRadius: 4, padding: "5px 12px", color: "#4a6378", cursor: "pointer", fontSize: 10, fontFamily: "monospace", minHeight: 34 }}>
                  ⬇ .md
                </button>
              </div>
              <div style={{ background: "#0d1117", border: "1px solid #1e2d3d", borderRadius: 6, padding: isNarrow ? "12px 14px" : "16px 20px", color: "#c9d1d9", fontSize: 12, lineHeight: 1.8, whiteSpace: "pre-wrap", fontFamily: "monospace" }}>
                {displayReport.text}
              </div>
          ...
          {!loading && !displayReport && (
            <div style={{ textAlign: "center", marginTop: 60 }}>
              {!hasApiKey ? (
                <div style={{ display: "inline-block", background: "#f59e0b11", border: "1px solid #f59e0b44", borderRadius: 6, padding: "20px 24px", maxWidth: 340 }}>
      ...
      {/* Right: history panel — collapsible on narrow */}
      {showHistoryPanel && (
        <div style={{ width: isNarrow ? 200 : 240, borderLeft: "1px solid #1e2d3d", background: "#0d1117", display: "flex", flexDirection: "column", flexShrink: 0 }}>
          ...
          <div className="scroll-touch" style={{ flex: 1, overflowY: "auto" }}>
            {history.length === 0 ? (
              ...
              <div key={h.id}
                ...
                <div style={{ color: "#4a6378", fontSize: 9 }}>{h.industry}</div>
          ...
          {history.length > 0 && (
            <div style={{ padding: "8px 12px", borderTop: "1px solid #1e2d3d" }}>
              <button onClick={() => { setHistory([]); saveHistory([]); setSelHistory(null); }}
                style={{ background: "transparent", border: "1px solid #1e2d3d", borderRadius: 3, padding: "5px 8px", color: "#3d5168", cursor: "pointer", fontSize: 9, width: "100%", fontFamily: "monospace", minHeight: 32 }}>
                履歴をクリア
              </button>
            </div>
          )}
